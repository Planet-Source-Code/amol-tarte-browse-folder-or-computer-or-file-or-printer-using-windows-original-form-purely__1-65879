Attribute VB_Name = "modLocalBrowser"
Option Explicit
...
Public Function BrowseForComputer(ByVal Hndl As Long, sSelPath As String _
, ByVal Message As String, Optional DontGoBelowDomain As Boolean) As String
    ...
    With BIF
        ...
        CopyMemory ByVal lpSelPath, ByVal sSelPath, Len(sSelPath)
        .lParam = lpSelPath
        .ulFlags = BIF_BROWSEFORCOMPUTER _
        + IIf(DontGoBelowDomain = True, BIF_DONTGOBELOWDOMAIN, 0&)
     End With
    ...
    If pidl Then
        If SHGetPathFromIDList(pidl, sPath) Then
           BrowseForComputer = sPath 'Left$(sPath, InStr(sPath, vbNullChar) - 1)
        End If
        CoTaskMemFree pidl
    End If
    LocalFree lpSelPath
...
Public Function BrowseForPrinter(ByVal Hndl As Long, sSelPath As String _
, ByVal Message As String, Optional DontGoBelowDomain As Boolean) As String
    ...
    With BIF
        ...
        CopyMemory ByVal lpSelPath, ByVal sSelPath, Len(sSelPath)
        .lParam = lpSelPath
        .ulFlags = BIF_BROWSEFORPRINTER _
        + IIf(DontGoBelowDomain = True, BIF_DONTGOBELOWDOMAIN, 0&)
     End With
    ...
    If pidl Then
        If SHGetPathFromIDList(pidl, sPath) Then
           BrowseForPrinter = Left$(sPath, InStr(sPath, vbNullChar) - 1)
        End If
        CoTaskMemFree pidl
    End If
    LocalFree lpSelPath
